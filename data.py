...
class ODEDataset(IterableDataset):
    ...
    def __init__(self, dxdt, dim, num_examples, x0_lim, dt=1e-2, seq_len=100):
        self.dxdt = dxdt
        self.dim = dim
    ...
    def __iter__(self):
        ...
        for x0 in x0s:
            ...
            yield (x0.astype(np.float32), res.y.T.astype(np.float32))


class ODEDataModule(pl.LightningDataModule):
    def __init__(self, dxdt, dim, x0_lim, batch_size):
        super().__init__()
        self.dxdt = dxdt
        self.dim = dim
        self.x0_lim = x0_lim
    ...
    def setup(self, stage: str):
        del stage
        self.train_dset = ODEDataset(self.dxdt, self.dim, self.batch_size * 1000, self.x0_lim, dt=1e-2, seq_len=100)
        self.val_dset = ODEDataset(self.dxdt, self.dim, self.batch_size * 100, self.x0_lim, dt=1e-2, seq_len=100)
        self.test_dset = ODEDataset(self.dxdt, self.dim, self.batch_size * 100, self.x0_lim, dt=1e-2, seq_len=100)
